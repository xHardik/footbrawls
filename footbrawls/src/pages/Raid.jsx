...
export default function Raid() {
  ...
  if (!user) {
    return (
      <div style={s.page}>
        <p style={{ color: R.muted }}>Please complete onboarding first.</p>
      ...
      <main style={s.main}>
            ...
            <div style={s.modeGrid}>
              {Object.values(RAID_TYPES).map(mode => (
                <button key={mode.id} type="button" style={{
                  ...s.modeCard,
                  borderColor: mode.id === 'challenge' ? R.purple : R.border,
        ...
        {phase === 'results' && (
          <div style={s.section}>
            <div style={{
              ...s.outcomeBanner,
              borderColor: outcome === 'win' ? R.green : outcome === 'loss' ? R.red : R.muted,
              color: outcome === 'win' ? R.green : outcome === 'loss' ? R.red : R.muted,
            }}>
              ...
              {['act1', 'act2', 'act3'].map((key, i) => {
                ...
                return (
                  <div key={key} style={s.actRow}>
                    <span>Act {i + 1}</span>
                    <span style={{ color: w === 'you' ? R.green : w === 'rival' ? R.red : R.muted }}>
                ...
                {finalizing ? (
                  ...
                  <>
                    ...
                    {finalizeResult?.xpResults?.mvp?.xpAwarded > 0 && (
                      <div style={{ color: R.accent, marginTop: 6 }}>+{finalizeResult.xpResults.mvp.xpAwarded} MVP</div>
                    )}
                  </>
                )}
              </div>
...
const s = {
  page:          { minHeight: '100vh', background: R.bg, color: R.text, fontFamily: "'Syne', sans-serif", paddingBottom: 40 },
  nav:           { display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 20px', height: 60, borderBottom: `1px solid ${R.border}`, background: 'rgba(6,8,16,0.9)', backdropFilter: 'blur(12px)', position: 'sticky', top: 0, zIndex: 100 },
  backBtn:       { background: 'none', border: 'none', color: R.muted, fontSize: 28, cursor: 'pointer', padding: 0 },
  guildBtn:      { background: R.surface, border: `1px solid ${R.border}`, borderRadius: 10, padding: '6px 10px', cursor: 'pointer', fontSize: 18 },
  logo:          { fontFamily: "'Bebas Neue', sans-serif", fontSize: '1.5rem', letterSpacing: 3, color: R.purple },
  ...
  desc:          { color: R.muted, fontSize: '0.9rem', lineHeight: 1.6, marginBottom: 24 },
  modeGrid:      { display: 'flex', flexDirection: 'column', gap: 12 },
  modeCard:      { padding: '18px 16px', background: R.surface, border: '1px solid', borderRadius: 14, cursor: 'pointer', textAlign: 'left', color: R.text, transition: 'border-color 0.2s' },
  modeLabel:     { fontFamily: "'Bebas Neue', sans-serif", fontSize: '1.3rem', letterSpacing: 1.5, marginBottom: 6 },
  modeXp:        { fontSize: '0.78rem', color: R.muted },
  modeDmg:       { fontSize: '0.68rem', color: R.purple, marginTop: 6, fontFamily: "'Space Mono', monospace", letterSpacing: 0.5 },
  center:        { textAlign: 'center', padding: '60px 0' },
  spinner:       { width: 40, height: 40, borderRadius: '50%', border: '3px solid rgba(255,255,255,0.08)', borderTopColor: R.purple, margin: '0 auto 20px', animation: 'spin 0.8s linear infinite' },
  searchTitle:   { fontFamily: "'Bebas Neue', sans-serif", fontSize: '1.8rem', letterSpacing: 2, marginBottom: 8 },
  muted:         { color: R.muted, fontSize: '0.82rem' },
  teams:         { display: 'flex', alignItems: 'center', gap: 12, marginTop: 20 },
  teamBox:       { flex: 1, padding: 14, background: R.surface, border: `1px solid ${R.border}`, borderRadius: 14 },
  teamHdr:       { fontFamily: "'Space Mono', monospace", fontSize: '0.58rem', letterSpacing: 2, color: R.muted, marginBottom: 10, textTransform: 'uppercase' },
  playerRow:     { display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8, fontSize: '0.85rem' },
  botTag:        { fontSize: '0.55rem', padding: '2px 6px', borderRadius: 99, background: 'rgba(168,85,247,0.15)', color: R.purple, fontWeight: 700 },
  vs:            { fontFamily: "'Bebas Neue', sans-serif", fontSize: '1.2rem', color: R.muted },
  ...
  actSummary:    { background: R.surface, border: `1px solid ${R.border}`, borderRadius: 14, padding: 16, marginBottom: 20 },
  actRow:        { display: 'flex', justifyContent: 'space-between', padding: '8px 0', fontSize: '0.88rem', borderBottom: `1px solid ${R.border}` },
  xpBox:         { textAlign: 'center', padding: 16, background: 'rgba(61,214,140,0.08)', border: `1px solid rgba(61,214,140,0.25)`, borderRadius: 12, marginBottom: 16, fontWeight: 800, fontSize: '1.2rem', color: R.green },
  curseLift:     { textAlign: 'center', padding: 12, background: 'rgba(247,195,68,0.1)', border: '1px solid rgba(247,195,68,0.3)', borderRadius: 12, color: R.accent, marginBottom: 16, fontWeight: 700 },
  primaryBtn:    { width: '100%', padding: 16, background: `linear-gradient(135deg, ${R.purple}, #7c3aed)`, color: '#fff', border: 'none', borderRadius: 14, fontWeight: 800, cursor: 'pointer', marginBottom: 10, letterSpacing: 1 },
  secondaryBtn:  { width: '100%', padding: 14, background: 'transparent', color: R.muted, border: `1px solid ${R.border}`, borderRadius: 14, cursor: 'pointer', fontWeight: 600 },
};
