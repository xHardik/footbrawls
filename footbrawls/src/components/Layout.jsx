...
export default function Layout({ children, hideMobileNav }) {
  ...
  const handleRaidTimeout = useCallback((session) => {
    try {
      if (session.currentAct === 1) {
        session.scores.act1 = {
          gameId: session.act1Game?.id || 'whoareya_correct',
          rawScore: 0,
          normalized: 0
        };
      ...
      delete session.actStartedAt;
      delete session.lastActForTimer;

      localStorage.setItem('active_raid_session', JSON.stringify(session));
      window.location.href = '/raid';
    } catch (e) {
      console.error('[Layout] Raid timeout handler error:', e);
    }
  }, []);
  ...
  useEffect(() => {
      ...
      try {
        ...
        if (sessionStr) {
          ...
          if (session && session.active) {
            // Set start time if missing or changed act
            if (!session.actStartedAt || session.lastActForTimer !== session.currentAct) {
              session.actStartedAt = Date.now();
              session.lastActForTimer = session.currentAct;
              localStorage.setItem('active_raid_session', JSON.stringify(session));
            }
            setRaidSession(session);

             const limit = 60;
             const elapsed = Math.floor((Date.now() - session.actStartedAt) / 1000);
             const remaining = Math.max(0, limit - elapsed);
             setSecondsLeft(remaining);
 
             if (remaining <= 0) {
               handleRaidTimeout(session);
             }
             return;
           }
         }
         ...
         console.error('[Layout] Error checking raid session:', e);
         setRaidSession(null);
       }
     };
 
     checkRaid();
     const timerId = setInterval(checkRaid, 1000);
   ...
   if (raidSession) {
     ...
     const match = raidSession.match;

    return (
      <div className="ly-root-raid" style={{ display: 'flex', flexDirection: 'column', minHeight: '100vh', background: C.bg, color: C.text, fontFamily: "'Syne', sans-serif" }}>
        {/* Premium Raid HUD Header */}
        <div style={{
          background: 'linear-gradient(90deg, #0c0f1a, #161026, #0c0f1a)',
          borderBottom: `1px solid ${isLowTime ? C.red : 'rgba(168, 85, 247, 0.4)'}`,
          padding: '12px 20px',
          boxShadow: isLowTime ? '0 0 20px rgba(232, 64, 64, 0.15)' : '0 4px 30px rgba(0,0,0,0.4)',
          position: 'sticky',
          top: 0,
          zIndex: 1000,
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 10 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              <span style={{
                ...
                fontSize: '1.4rem',
                letterSpacing: 2,
                color: isLowTime ? C.red : C.accent,
                display: 'flex',
                alignItems: 'center',
                gap: 6
              }}>
                ⚔️ RAID ACT {raidSession.currentAct}
              </span>
              <span style={{
                ...
                padding: '2px 8px',
                borderRadius: 4,
                background: isTraining ? 'rgba(255,255,255,0.06)' : 'rgba(168, 85, 247, 0.15)',
                color: isTraining ? C.muted : C.purple,
                border: `1px solid ${isTraining ? C.border : 'rgba(168, 85, 247, 0.3)'}`,
                textTransform: 'uppercase',
                fontWeight: 'bold'
              }}>
                {raidSession.raidType} MODE
              </span>
            </div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              <div style={{
                ...
                fontSize: '1.2rem',
                fontWeight: 'bold',
                color: isLowTime ? C.red : C.text,
                background: isLowTime ? 'rgba(232, 64, 64, 0.1)' : 'rgba(255,255,255,0.03)',
                padding: '4px 12px',
                borderRadius: 8,
                border: `1px solid ${isLowTime ? C.red : C.border}`,
                boxShadow: isLowTime ? '0 0 10px rgba(232, 64, 64, 0.2)' : 'none',
                minWidth: 50,
                textAlign: 'center',
                animation: isLowTime ? 'pulse 1s infinite alternate' : 'none'
              }}>
                ⏱️ {secondsLeft}s
              </div>
            </div>
          ...
          {/* Header row 2: Roster match visualization */}
          {match && (
            <div style={{
              display: 'flex',
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                <span style={{ opacity: 0.6 }}>Your Duo:</span>
                <span style={{ fontWeight: 'bold' }}>{user?.flag} {user?.nickname}</span>
                <span style={{ opacity: 0.4 }}>&</span>
                <span style={{ fontWeight: 'bold' }}>{match.buddy?.flag} {match.buddy?.nickname}</span>
              </div>
              ...
              {/* Rivals */}
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                <span style={{ opacity: 0.6 }}>Rivals:</span>
                {match.rivals?.map((rival, ri) => (
                  <span key={rival.userId || ri} style={{ fontWeight: 'bold' }}>
                    {rival.flag} {rival.nickname}
                    {ri < match.rivals.length - 1 && <span style={{ fontWeight: 'normal', opacity: 0.4, marginLeft: 6 }}>&</span>}
                  </span>
                ))}
              </div>
            </div>
          ...
          {/* Pulsing Timer Progress Bar */}
          <div style={{ height: 4, background: 'rgba(255,255,255,0.08)', borderRadius: 2, overflow: 'hidden', marginTop: 10 }}>
            <div style={{
              width: `${timerPercent}%`,
              height: '100%',
              background: isLowTime ? C.red : `linear-gradient(90deg, ${C.accent}, ${C.purple})`,
              transition: 'width 1s linear',
            }} />
          </div>
        </div>

        {/* CSS Keyframes for Pulsing Timer */}
        <style>{`
          @keyframes pulse {
        ...
        {/* Content Container (Full Width without Sidebar) */}
