 ...
 * UI theme ported from Wordle.jsx (Crickingo fidelity: nav, puzzle bar, score box, modal, dashboard, bottom nav)
...
.pn-tag-dot {
  ...
  background: var(--accent3); animation: pnBlink 1.5s ease infinite;
}
...
.pn-help-tooltip::after {
  ...
  border-left-color: rgba(232,64,64,0.25);
}
.pn-help-wrap:hover .pn-help-tooltip {
...
.pn-msg {
  ...
  margin-bottom: 18px; animation: pnFadeUp 0.3s ease; border: 1px solid;
}
...
.pn-progress-dot.dot-win {
  ...
  box-shadow: 0 0 10px rgba(61,214,140,0.2);
}
...
.pn-goal-zone.zone-aimed {
  ...
  box-shadow: inset 0 0 15px rgba(232,64,64,0.2);
}
.pn-goal-zone.zone-gk-dived {
  background: rgba(255,164,0,0.12);
  border: 1px solid rgba(255,164,0,0.45);
  box-shadow: inset 0 0 15px rgba(255,164,0,0.25);
}
.pn-goal-zone.zone-ball-scored {
  background: rgba(61,214,140,0.18);
  border: 1px solid rgba(61,214,140,0.5);
  box-shadow: inset 0 0 20px rgba(61,214,140,0.3);
}
.pn-goal-zone:hover { border-color: rgba(255,255,255,0.2); }

.pn-gk-dive-badge {
  background: var(--accent2); color: #FFF;
  font-family: 'DM Sans', sans-serif; font-size: 0.65rem; font-weight: 900;
  padding: 4px 8px; border-radius: 4px;
  box-shadow: 0 4px 8px rgba(0,0,0,0.3); letter-spacing: 0.5px;
}
.pn-ball-goal-badge {
...
/* ── KICK BUTTON ── */
.pn-kick-btn {
  background: var(--accent3); color: #fff; padding: 13px 28px; width: 100%;
  border-radius: 12px; border: none;
  font-family: 'DM Sans', sans-serif; font-size: 0.95rem; font-weight: 800;
  cursor: pointer; text-transform: uppercase; letter-spacing: 1px;
  transition: all 0.22s ease; box-shadow: 0 4px 16px rgba(232,64,64,0.28);
}
...
.pn-result {
  ...
  animation: pnFadeUp 0.5s ease; position: relative; overflow: hidden;
}
.pn-result::before {
...
.pn-result-score {
  ...
  background: linear-gradient(135deg, var(--accent3), #ffb3b3 60%);
  -webkit-background-clip: text; -webkit-text-fill-color: transparent; background-clip: text;
  line-height: 1; margin: 12px 0;
  filter: drop-shadow(0 0 22px rgba(232,64,64,0.4));
  animation: pnScorePulse 2.5s ease-in-out infinite;
}
@keyframes pnScorePulse {
...
.pn-replay-status-badge {
  font-family: 'DM Sans', sans-serif; font-size: 0.62rem; font-weight: 900;
...
.pn-ad-go-btn {
  background: var(--accent3); color: #fff; width: 100%;
  justify-content: center; box-shadow: 0 4px 16px rgba(232,64,64,0.28);
  padding: 12px; display: inline-flex; align-items: center; gap: 8px;
  border: none; border-radius: 12px; cursor: pointer;
  font-family: 'DM Sans', sans-serif; font-weight: 800; font-size: 0.85rem;
  text-transform: uppercase; letter-spacing: 1px; transition: all 0.22s ease;
}
...
.pn-modal-box {
  ...
  position: relative; animation: pnModalUp 0.32s cubic-bezier(0.4,0,0.2,1);
}
.pn-modal-box::before {
  content: ''; position: absolute; top: 0; left: 0; right: 0; height: 3px;
  background: linear-gradient(90deg, var(--accent3), var(--accent), var(--accent2));
...
.pn-dash-card::before {
  content: ''; position: absolute; inset: 0;
  background: linear-gradient(135deg, rgba(232,64,64,0.03), transparent 60%); pointer-events: none;
}
...
.pn-stat-val {
  ...
  background: linear-gradient(135deg, var(--accent3), #ffb3b3 80%);
  -webkit-background-clip: text; -webkit-text-fill-color: transparent; background-clip: text;
...
export default function PenaltyNerve({ onBack }) {
          ...
          try {
            ...
            if (!user?.userId) {
              ...
              if (res?.cappedOut) {
                ...
                finalAwarded = res?.xpAwarded ?? finalXP;
              }
            }
          } catch (err) {
  ...
  return (
    <div style={{ background: "var(--bg,#05070f)", minHeight: "100vh", color: "var(--text,#F0F0F0)", fontFamily: "'DM Sans',sans-serif" }}>
      ...
      {showModal && (
        <HowToPlayModal onClose={() => setShowModal(false)} />
      )}
                ...
                <div className="pn-goal-frame">
                  ...
                  >
                    <svg viewBox="0 0 64 64" className="pn-gk-svg" xmlns="http://www.w3.org/2000/svg">
                      {/* Ground shadow */}
                      <ellipse className="pn-gk-shadow" cx="32" cy="61" rx="14" ry="2.5" />

                      {/* Right arm (behind torso) */}
                      <rect className="pn-gk-arm pn-gk-arm-right" x="38" y="22" width="8" height="18" rx="4" />
                      <circle className="pn-gk-glove" cx="42" cy="40" r="4.5" />

                      {/* Left arm (behind torso) */}
                      <rect className="pn-gk-arm pn-gk-arm-left" x="18" y="22" width="8" height="18" rx="4" />
                      <circle className="pn-gk-glove" cx="22" cy="40" r="4.5" />

                      {/* Legs / boots */}
                      <rect className="pn-gk-leg pn-gk-leg-left"  x="24" y="42" width="6.5" height="15" rx="2.5" />
                      <rect className="pn-gk-leg pn-gk-leg-right" x="33.5" y="42" width="6.5" height="15" rx="2.5" />
                      <ellipse className="pn-gk-boot" cx="27.2" cy="58.5" rx="5" ry="2.5" />
                      <ellipse className="pn-gk-boot" cx="36.8" cy="58.5" rx="5" ry="2.5" />

                      {/* Shorts */}
                      <rect className="pn-gk-shorts" x="22" y="34" width="20" height="11" rx="4" />

                      {/* Jersey / torso */}
                      <rect className="pn-gk-jersey" x="20" y="16" width="24" height="20" rx="6" />
                      <text className="pn-gk-number" x="32" y="30">1</text>

                      {/* Neck + head */}
                      <rect className="pn-gk-skin" x="29" y="11" width="6" height="6" rx="2" />
                      <circle className="pn-gk-skin" cx="32" cy="9" r="7" />
                      <path className="pn-gk-hair" d="M25 8.5 a7 7 0 0 1 14 0 q-1 -3 -3.2 -3.4 q-1.6 1.6 -3.8 1 q-2 -1 -3.6 0.2 q-2 1 -3.4 2.2 Z" />
                    </svg>
                  </div>
                </div>
